  ...
  //  check_field("count_id");
    ...
    //end
			...
			//if(confirm("Do You Wants to Save Record ?")){
				e.preventDefault();
				data = new FormData($('#chalan-form')[0]);//form name
        /*Check XSS Code*/
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
				$.ajax({
				type: 'POST',
				url: base_url+'sales/save_chalan?command='+this_id+'&rowcount='+rowcount,
				data: data,
				cache: false,
				contentType: false,
				processData: false,
         ...
         // alert(result);return;
				result=result.split("<<<###>>>");
					if(result[0]=="success")
					{
						location.href=base_url+"sales/chalan_invoice/"+result[1];
					}
...
function return_row_with_data1(item_id){
  ...
  $.post(base_url+"sales/return_row_with_data1/"+rowcount+"/"+item_id,{customer_id:customer_id,warehouse_id:warehouse_id},function(result){
        //alert(result);
