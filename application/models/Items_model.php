...
class Items_model extends CI_Model {
	...
	public function save_record($modal_post=array()){
		...
		if($item_group=='Single'){
			...
			//single item cloud entry starts
			try {
				
				// if (!empty($_FILES['item_image']['name'])) {

				// 	// instead of CodeIgniter's upload library, manually move file to cloud uploads folder
				// 	$tmp_name = $_FILES['item_image']['tmp_name'];
				// 	$original_name = $_FILES['item_image']['name'];

				// 	// Set your cPanel username and domain properly
				// 	$destination_path = '/admin.megasgoods.com/media/item_photos/' . $original_name;


				// 	if (move_uploaded_file($tmp_name, $destination_path)) {
				// 		$file_name = $original_name; // Save the file name if uploaded successfully
				// 	} else {
				...
				$cloud_data = [
					...
					'title'             => $item_name,
					'description'       => $description,
					'item_photo'        => (!empty($file_name)) ? 'uploads/items/'.$file_name : '',
					'item_old_price'    => $mrp,
					'discount'          => $discount,
					'item_new_price'    => $sales_price,
					'status'            => 1, // Active by default
					'created_at'        => date('Y-m-d H:i:s'),
					'deal_of_the_day'   => isset($deal_of_the_day) ? $deal_of_the_day : 0,
					'recommended'       => isset($recommended) ? $recommended : 0,
					'most_popular'      => isset($most_popular) ? $most_popular : 0,
					'category_id'       => $category_id,
					'makingprice'       => $purchase_price,
					'veg'               => isset($veg) ? $veg : 0,
					'non_veg'           => isset($non_veg) ? $non_veg : 0,
					'flag'              => 1,
					'piece_count'	    => 	$piece_count,
				...
				// Check if the item already exists in the cloud DB
				$cloud_item = $this->cloud_db->select('id')
								 ->from('menu_management_item')
								 ->where('id', $item_id)
								 ->get();
			
				if ($cloud_item->num_rows() > 0) {
					// Update existing item
					$cloud_item_id = $cloud_item->row()->id;
			
					// Remove created_at for update
					unset($cloud_data['created_at']);
			
					$this->cloud_db->where('id', $cloud_item_id)->update('menu_management_item', $cloud_data);

					$stock_data = [
						'openingstock' => $adjustment_qty,  // Ensure you have this in your form
						'item_id_id'   => $cloud_item_id,
						'piece_count' => $piece_count,
						'created_at'   => date('Y-m-d H:i:s'),
					...
					$cloud_stock = $this->cloud_db->select('id')
						->from('ecomApp_stock')
						->where('item_id_id', $cloud_item_id)
						->get();

					if ($cloud_stock->num_rows() > 0) {
						// Update existing stock in cloud
						$this->cloud_db->where('item_id_id', $cloud_item_id)->update('ecomApp_stock', $stock_data);
					} else {
						// Insert new stock in cloud
						$this->cloud_db->insert('ecomApp_stock', $stock_data);
					}

					$local_stock = $this->db->select('id')
					->from('ecomApp_stock')
					->where('item_id_id', $cloud_item_id)
					->get();

					if ($local_stock->num_rows() > 0) {
						$this->db->where('item_id_id', $cloud_item_id)->update('ecomApp_stock', $stock_data);
					} else {
						$this->db->insert('ecomApp_stock', $stock_data);
					}
				
					//$this->cloud_db->insert('ecomApp_stock', $stock_data);
					//echo "UPDATE QUERY: " . $this->cloud_db->last_query() . "<br>";
				} else {
					// Insert new item
					$this->cloud_db->insert('menu_management_item', $cloud_data);
					$inserted_item_id = $this->cloud_db->insert_id();

				// Prepare stock data
				$stock_data = [
					'openingstock' => $adjustment_qty,  // assuming you have this input
					'item_id_id'   => $inserted_item_id,
					'piece_count' => $piece_count,
					'created_at'   => date('Y-m-d H:i:s'),
					'updated_at'   => date('Y-m-d H:i:s'),
					'flag'         => 1,
				...
				// Insert into ecomApp_stock table
				$this->cloud_db->insert('ecomApp_stock', $stock_data);
				$this->db->insert('ecomApp_stock', $stock_data);
					//echo "INSERT QUERY: " . $this->cloud_db->last_query() . "<br>";
				}
			
			} catch (Exception $e) {
				log_message('error', 'Cloud DB Insert/Update Failed: ' . $e->getMessage());
				// Optional: rollback main DB insert/update
			}
			//single item cloud entry ends
		}//Single END
					...
					if(isset($_REQUEST['tr_variant_id_'.$i]) && !empty($_REQUEST['tr_variant_id_'.$i])){
						...
						$info = array(
							...
							try {
								$cloud_data = [
									'id'               => $variant_item_id,
									'title'             => $item_name."-".$variant_name,
									'description'       => $description,
									'item_photo'        => (!empty($file_name)) ? 'uploads/items/'.$file_name : '',
									'item_old_price'    => $item_mrp,
									'discount'          => $discount,
									'item_new_price'    => $sales_price,
									'status'            => 1, // Active by default
									'created_at'        => date('Y-m-d H:i:s'),
	...
	public function delete_items_from_table($ids){
		...
		// Step 3: Delete from cloud db (menu_management_item and ecomApp_stock)
		try {
			$itemIds = explode(',', $ids);

			// Delete related stock entries first
			$this->cloud_db->where_in('item_id_id', $itemIds);
			$this->cloud_db->delete('ecomApp_stock');

			// Then delete the items
			$this->cloud_db->where_in('id', $itemIds); // assuming 'id' is the PK in menu_management_item
			$this->cloud_db->delete('menu_management_item');

		} catch (Exception $e) {
			log_message('error', 'Failed to delete from cloud DB: ' . $e->getMessage());
			$this->db->trans_rollback();
			echo "Cloud Delete Failed";
			exit;
		}
