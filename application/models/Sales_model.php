...
class Sales_model extends CI_Model {
	...
	public function add_brand($array){
	    if($this->db->insert('db_brands',$array)){
	        return true;
	    }
	    else{
	        return false;
	...
	public function add_category($array){
	    if($this->db->insert('db_category',$array)){
	        return true;
	    }
	    else{
	        return false;
	...
	public function verify_save_and_update(){
	    ...
	    if($command=='save'){//Create sales code unique if first time entry
		    ...
		    $sales_entry = array(
		    				...
		    				/*Add Interest*/
		    				'add_interest' 	=> (($add_interest)?'YES':'NO'),
							'interest_percent' 	=> (($add_interest)?12:0),
							'interest_update' 	=> (($add_interest)?date('Y-m-d H:i:s'):NULL),
		    				...
		    				'status' 					=> 1,
		    				'finance_id'              => NULL, //$finance_name,
		    				'finance_amount'              => 0, //$finance_amount,
		    				'total_emi_month'              => 0, //$total_emi_month,
		    				'emi_per_month'              => 0, //$emi_per_month,
		    			);
		   ...
		   // print_r($sales_entry);exit;
			...
			$sales_entry = array(
		    				...
		    				'total_emi_month'              => $total_emi_month,
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				//$serial_no		=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_22']));
				//$serial_no			=$this->xss_html_filter(trim(((count($_REQUEST['td_data_'.$i.'_22'])==1)?$_REQUEST['td_data_'.$i.'_22']:implode(',',$_REQUEST['td_data_'.$i.'_22']))));
	...
	public function get_items_info($rowcount,$item_id){
		...
		$res4=$this->db->select('*')->from('db_units')->where("id=$res1->unit_id")->get()->row();
		...
		$this->return_row_with_data1($rowcount,$info);
	}
	public function get_items_info1($rowcount,$item_id){
		extract($_POST);
	...
	public function return_sales_list($sales_id){
		...
		foreach ($q1->result() as $res1) {
			$res2=$this->db->query("select * from db_items where id=".$res1->item_id)->row();
			$res3=$this->db->query("select * from db_units where id=".$res2->unit_id)->row();
			$q3=$this->db->query("select * from db_tax where id=".$res1->tax_id)->row();
			...
			$info = array(
							...
							'item_price' 				=> $res2->price,
							'item_unit' 				=> $res3->unit_name,
							'item_sales_price' 			=> $res1->price_per_unit, 
							'item_sales_qty' 			=> $res1->sales_qty,
							'item_tax_name' 			=> $q3->tax_name, 
							'item_serial_no' 			=> $res1->serial_no, 
							'item_tax_id' 				=> $res1->tax_id, 
							'item_tax' 					=> $q3->tax, 
							'item_tax_type' 			=> $res1->tax_type, 
							'item_tax_amt' 				=> $res1->tax_amt, 
	...
	public function return_row_with_data($rowcount,$info){
		extract($info);
		$saleSerialCount = $this->db->query("SELECT COUNT(serial_no) cnt FROM `db_salesitems` WHERE item_id='$item_id' AND serial_no <> ''")->row()->cnt;
		if($saleSerialCount)
		    $itemSerials = $this->db->query("SELECT id, serial_no, DATEDIFF(CURRENT_DATE, added_date) totdate FROM `db_item_serials` di WHERE item_id='$item_id' AND  NOT FIND_IN_SET(di.serial_no, (SELECT IFNULL((SELECT GROUP_CONCAT(serial_no) FROM `db_salesitems` WHERE item_id='$item_id' AND serial_no <> ''), ''))) ORDER BY added_date ASC")->result();
		else
		    $itemSerials = $this->db->query("SELECT id, serial_no FROM `db_item_serials` di WHERE item_id='$item_id' ORDER BY added_date ASC")->result();
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_22">
                   <?php 
                        if($itemSerials){
                            echo '<select class="form-control select2" id="td_data_'.$rowcount.'_22" name="td_data_'.$rowcount.'_22[]" multiple="multiple">';
                                foreach($itemSerials as $is)
                                    echo '<option value="'.$is->serial_no.'">'.$is->serial_no.'</option>';
                            echo '</select>';
                        }
                        else{
                    ?>
                       <input style="width:250px !important;" type="text" name="td_data_<?=$rowcount;?>_22" id="td_data_<?=$rowcount;?>_22[]" class="form-control text-right no-padding only_currency text-center"  placeholder="serial1, serial2" value="<?=$item_serial_no;?>">
                   <?php } ?>
                </td>
                */ ?>
               <!-- Qty -->
               ...
               <!-- Unit-->
               <td id="td_<?=$rowcount;?>_18"><input type="text" name="td_data_<?=$rowcount;?>_18" id="td_data_<?=$rowcount;?>_18" readonly class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" value="<?= $item_unit;?>"></td>
	...
	///------------------------------------------------For double sale item in add sale------------------------------//
	/*
	public function return_row_with_data1($rowcount,$info){
		extract($info);
		$item_amount = ($item_sales_price * $item_sales_qty) + $item_tax_amt;
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_1">
                  <label class='form-control' style='height:auto;width:250px;display: block;' data-toggle="tooltip" title='Edit ?' >
                  <a id="td_data_<?=$rowcount;?>_1" href="javascript:void()" onclick="show_sales_item_modal(<?=$rowcount;?>)" title=""><?=$item_name;?></a> 
                  		<i onclick="show_sales_item_modal(<?=$rowcount;?>)" class="fa fa-edit pointer"></i>
                  	</label>
               </td>
 ...
 */
	...
	public function verify_chalan(){
	    ...
	    if($command=='save'){//Create sales code unique if first time entry
			...
			$this->db->query("ALTER TABLE db_chalan AUTO_INCREMENT = 1");
		   ...
		   // print_r($sales_entry);exit;
			$q1 = $this->db->insert('db_chalan', $sales_entry);
			$sales_id = $this->db->insert_id();
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$q2 = $this->db->insert('db_chalan_item', $salesitems_entry);
