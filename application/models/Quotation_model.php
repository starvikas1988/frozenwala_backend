	...
	public function verify_save_and_update(){
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$serial_no  		=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_14']));
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <!-- <td id="td_<?=$rowcount;?>_17">
                 ...
                 <!-- Serial Number-->
               <td id="td_<?=$rowcount;?>_14"><input type="text" name="td_data_<?=$rowcount;?>_14" id="td_data_<?=$rowcount;?>_14" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" value="<?=$item_serial_no;?>"></td>
               <!-- Qty -->
