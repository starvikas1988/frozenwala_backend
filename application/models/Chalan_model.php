...
*/
	...
	public function get_details($id,$data){
		...
		$query=$this->db->query("select * from db_chalan where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
	...
	public function return_row_with_data1($rowcount,$info){
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_1">
                  <label class='form-control' style='height:auto;width:700px;display: block;' data-toggle="tooltip" title='Edit ?' >
                  <a id="td_data_<?=$rowcount;?>_1" href="javascript:void()" onclick="show_sales_item_modal(<?=$rowcount;?>)" title=""><?=$item_name;?></a> 
                  		<i onclick="show_sales_item_modal(<?=$rowcount;?>)" class="fa fa-edit pointer"></i>
                  	</label>
               </td>
	...
	public function get_chalan(){
		$this->load->database();
		$this->db->select('*');
		//$this->db->join("db_chalan_item b", "b.chalan_id=a.id", "inner");
		$this->db->join('db_customers as c','c.id=a.customer_id','left');
		$this->db->from("db_chalan a");
