...
class Reports_model extends CI_Model {
	...
	public function show_supplier_items_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$this->db->select("a.unit_total_cost,a.purchase_qty,a.tax_amt,a.price_per_unit,b.store_id,b.warehouse_id, b.id,a.item_id,b.supplier_id,b.purchase_code,b.purchase_date");
				...
				if($q2->num_rows()>0){
					foreach ($q2->result() as $res2){
						...
						echo "<td class='text-right'>".store_number_format($res2->price_per_unit*$res2->purchase_qty)."</td>";
						echo "</tr>";
	...
	public function show_sales_report(){
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				echo '<td><div class="btn-group" title="View Account">
										<a class="btn btn-primary btn-o dropdown-toggle" data-toggle="dropdown" href="#">
											Action <span class="caret"></span>
										</a>
										<ul role="menu" class="dropdown-menu dropdown-light pull-right"><li>
												<a title="Pay" class="pointer" onclick="view_payments('.$res1->id.')" >
													<i class="fa fa-fw fa-money text-blue"></i>View Payments
												</a>
											</li>
											<li>
												<a title="Receive Payments" class="pointer" onclick="pay_now('.$res1->id.')" >
	...
	public function show_ledger_report(){
		extract($_POST);
	...
	public function show_stock_aging_report(){
		extract($_POST);
...
// 		die;
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				$serials = '';
				$serialData = $this->db->query("SELECT id, serial_no, DATEDIFF(CURRENT_DATE, added_date) totdate FROM `db_item_serials` di WHERE item_id='$res1->item_id' AND  NOT FIND_IN_SET(di.serial_no, (SELECT IFNULL((SELECT GROUP_CONCAT(serial_no) FROM `db_salesitems` WHERE item_id='$res1->item_id' AND serial_no <> ''), ''))) ORDER BY added_date ASC")->result();
				if($serialData)
				{
				    $serials .= '<table class="table table-bordered"><tr><th>No</th><th>Days</th></tr>';
				    
				    foreach($serialData as $slr)
				    {
				        $serials .= '<tr><td>'.$slr->serial_no.'</td><td>'.$slr->totdate.'</td></tr>';
				    }
				    $serials .='</table>';
				}
				$tax_type = ($res1->tax_type=='Inclusive') ? 'Inc.' : 'Exc.';
	...
	public function show_item_sales_report(){
		...
		if(!empty($brand_id)){
			$this->db->where("d.brand_id", $brand_id);
		}
