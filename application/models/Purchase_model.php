...
class Purchase_model extends CI_Model {
	...
	public function verify_save_and_update(){
		...
		// echo "<pre>";print_r($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));
		// echo"</pre>";
		// exit();
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				//insert item serials no
				if(isset($serial_no))
				{
					$serialArray = explode(',', $serial_no);
					if($serialArray)
					{
						foreach($serialArray as $res)
						{
						    $exist = $this->db->query("SELECT COUNT(id) cnt FROM `db_item_serials` WHERE item_id='$item_id' AND serial_no='".trim($res)."'")->row()->cnt;
						    if($exist)
						    {
						       $serialData = $this->db->query("SELECT id FROM `db_item_serials` WHERE item_id='$item_id' AND serial_no='".trim($res)."'")->row();
						       $update = [
						            'added_date' => (($pur_date)?$pur_date:date('Y-m-d')),
						            'updated_at' => date('Y-m-d H:i:s'),
						        ];
						       $this->db->update('db_item_serials', $update, ['id'=>$serialData->id]);
						    }
						    else
						    {
						        $itemSerial = [
    							...
    							$this->db->insert('db_item_serials', $itemSerial);
						    }
						}
					}					
				}
	...
	public function get_items_info($rowcount,$item_id){
		...
		$res2=$this->db->select('*')->from('db_units')->where("id=$res1->unit_id")->get()->row();
	...
	public function return_purchase_list($purchase_id){
		...
		foreach ($q1->result() as $res1) {
			...
			$res3=$this->db->select('*')->from('db_units')->where("id=$res2->unit_id")->get()->row();
			$q3=$this->db->query("select * from db_tax where id=".$res1->tax_id)->row();
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               <td id="td_<?=$rowcount;?>_1">
                 ...
                 <!-- Serial no-->
               <?php /*
               <td id="td_<?=$rowcount;?>_22"><input type="text" name="td_data_<?=$rowcount;?>_22" id="td_data_<?=$rowcount;?>_22" class="form-control text-right no-padding only_currency text-center"  value="<?=($item_serial_no ? $item_serial_no : '');?>" placeholder="serial1, serial2"></td>
                */?>
               <!-- Qty -->
               <td id="td_<?=$rowcount;?>_3">
                ...
                <!-- Unit -->
               <td id="td_<?=$rowcount;?>_11"><input type="text" name="td_data_<?=$rowcount;?>_11" readonly id="td_data_<?=$rowcount;?>_11" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" value="<?= $item_unit;?>" ></td>
               <!-- Purchase Price -->
