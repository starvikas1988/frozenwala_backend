...
class Category extends MY_Controller {
	...
	public function newcategory(){
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$result=$this->category_model->verify_and_save($_FILES);
			echo $result;
		} else {
	...
	public function update_category(){
		...
		if ($this->form_validation->run() == TRUE) {			
			$this->load->model('category_model');
			$result=$this->category_model->update_category($_FILES);
			echo $result;
		} else {
			echo "Please Enter Category name.";
		}
		...
		foreach ($list as $category) {
			...
			$row[] = $category->description;
				...
				if (!empty($category->image)) {
					$image_url = base_url('uploads/category/'.$category->image);
					$row[] = '<img src="'.$image_url.'" alt="Category Image" style="width:50px;height:50px;border-radius:4px;">';
				} else {
